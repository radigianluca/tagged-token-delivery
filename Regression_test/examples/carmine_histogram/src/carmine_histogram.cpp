...
#include <stdlib.h>
#include "carmine_histogram.h"

#define AMOUNT_OF_TEST 1

int carmine_histogram(in_int_t feature[1000], in_int_t weight[1000], inout_int_t hist[1000], in_int_t n) 
{
	int i;
	for (i=0; i<n; ++i) {
    int m = feature[i];
...
int main(void){
	...
	//for(int i = 0; i < AMOUNT_OF_TEST; ++i){
		int i = 0; 
		carmine_histogram(feature[i], weight[i], hist[i], n[i]);
	//}
}
